...
def update_sheet(aggregated_data, update_delta=False, history_data=None):
    ...
    if update_delta:
        ...
        try:
            ...
            prev_leads        = np.array(dashboard_sales.get("O2:O42", value_render_option=ValueRenderOption.unformatted))[:,0]
            prev_applications = np.array(dashboard_sales.get("T2:T42", value_render_option=ValueRenderOption.unformatted))[:,0]
        ...
        prev_leads_delta        = np.array(dashboard_sales.get("Q2:Q42", value_render_option=ValueRenderOption.unformatted))
        prev_applications_delta = np.array(dashboard_sales.get("V2:V42", value_render_option=ValueRenderOption.unformatted))
    ...
    if history_data is not None:
        dashboard_sales.update_acell('B46', str_date + ".2024") # 2024
        dashboard_sales.update_acell('B48', str_date + ".2023") # 2024
        dashboard_sales.update_acell('O46', str(history_data.loc[2024, 'leads'][0]))
        dashboard_sales.update_acell('O48', str(history_data.loc[2023, 'leads'][0]))
        dashboard_sales.update_acell('T46', str(history_data.loc[2024, 'applications']))
        dashboard_sales.update_acell('T48', str(history_data.loc[2023, 'applications']))
        dashboard_sales.update_acell('Y46', str(history_data.loc[2024, 'contracts']))
        dashboard_sales.update_acell('Y48', str(history_data.loc[2023, 'contracts']))
        dashboard_sales.update_acell('T45', str(history_data.loc[2025, 'applications_unique']))
        dashboard_sales.update_acell('T47', str(history_data.loc[2024, 'applications_unique']))
        dashboard_sales.update_acell('T49', str(history_data.loc[2023, 'applications_unique']))
